...
> * spring boot，目前使用该项目最新稳定版2.0
> * beetl，主要用于页面模板化，将页面划分成可重用的单元个体，并通过简单的表达式来达到自由组装页面布局的能力。除此之外，也提供对后端业务对象于页面对象的传递功能。
> * iBatis-Plus，基于iBatis核心框架，封装了部分对用户更为友好的功能，进一步降低开发人员使用iBatis的门槛，达到快速开发的能力。需要注意的是，该框架虽作为iBatis的框架增强，但也保持了原框架的结构，这也保证了Guns跟原生框架的兼容性。用户在使用过程中可根据自己的喜好或用户习惯进行自行决策。
> * shiro，AA集成框架，专注于集成，目前支持与多种主流用户数据进行集成，同时保留了其开放的一面。在项目中，用户只需要编写少量的代码，将自有数据桥接到框架对应的接口中，后面所有的事情将交给框架。
