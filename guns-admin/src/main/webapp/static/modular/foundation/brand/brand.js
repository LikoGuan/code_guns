...
Brand.initColumn = function () {
    return [
        {field: 'selectItem', radio: true},
            ...
            {title: '品牌描述', field: 'brandDesc', visible: true, align: 'center', valign: 'middle'},
            {title: '品牌描述', field: 'brandDesc', visible: true, align: 'center', valign: 'middle'}
    ];
};
...
Brand.openBrandDetail = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/brand/brand_update/' + Brand.seItem.brandId
        });
        this.layerIndex = index;
    }
};
...
Brand.delete = function () {
    if (this.check()) {
        ...
        ajax.set("brandId",this.seItem.brandId);
        ajax.start();
    }
};
